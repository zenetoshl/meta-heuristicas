...
def g4 (x):
    return (x[3] - 240)
def f(x):
    return (0.6224*x[0]*x[2]*x[3] + 1.7781*x[1]*(x[2]**2) + 3.1661*(x[0]**2)*x[3]+19.84*(x[0]**2)*x[2]) * check_restritions(x)

def check_restritions(x):
        multiplier = 1
        if(not (g1(x) <= 0 )):
            multiplier = multiplier + 10000000
        if(not (g2(x) <= 0) ):
            multiplier = multiplier + 10000000
        if(not (g3(x) <= 0) ):
            multiplier = multiplier + 10000000
...
best = 0
for i in range(100):
    solution = simulated_annealing([0, 0, 10, 10], [100, 100, 200, 200], f)
    if(i == 0):
        best = solution
    else:
